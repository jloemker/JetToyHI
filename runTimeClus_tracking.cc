...
//./runTimeClus_tracking -hard input/vectorTree_LHC25a2b.root -hardtype ROOT -hardtreename o2Tracks -hardvarname particle_truth -reco input/vectorTree_LHC25a2b.root -recotype ROOT -recotreename o2Tracks -recovarname particle_reco -nev 10000
...
int main (int argc, char ** argv) {
  ...
  TFile *fout = new TFile("JetToyHIResultTimeClus_tracking.root","RECREATE");
  ...
  {  
    ...
    vector<PseudoJet> particlesTruth = truth_selector(particlesMergedAll);// all input particles (PYTHIA/MC)

    // run as --reco
    fastjet::Selector detector_selector = SelectorVertexNumber(99);//how to get those from the real mc ??
    vector<PseudoJet> particlesDetectorFull = detector_selector(particlesMergedAll);

    //---------------------------------------------------------------------------
    //   fastsim
    //---------------------------------------------------------------------------
    //fastSim.setInputEvent(particlesAll);
    ...
    //vector<PseudoJet> particlesDetectorFull = fastSim.AliceDetector();// particles smeared by detector

    // reject randomly 3% of the tracks
    ...
    // FastSim is supposed to be in the MC !
    ...
    jetCollection jetCollectionTruth(sorted_by_pt(jet_selector(truth.inclusive_jets(15.))));

    fastjet::ClusterSequenceArea det(particlesDetector, jet_def, area_def);
    jetCollection jetCollectionDet(sorted_by_pt(jet_selector(det.inclusive_jets(15.))));
